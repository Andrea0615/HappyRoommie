...
export default function FAQ() {
  return (
    <section
      ...
      className="scroll-mt-20 bg-sand py-20 sm:py-28"
    ...
    >
      <div className="mx-auto grid max-w-6xl grid-cols-1 gap-10 px-4 sm:px-6 lg:grid-cols-[0.85fr_1.15fr] lg:px-8">
        <aside className="relative overflow-hidden rounded-[2rem] bg-ink p-8 text-white shadow-soft">
          ...
          <p className="relative mt-5 text-base leading-relaxed text-white/72">
        ...
        <div className="space-y-3">
          {FAQ_CONTENT.items.map((item) => (
            <details
              key={item.question}
              className="group rounded-[1.25rem] border border-ink/10 bg-white p-5 transition-all duration-300 open:shadow-card"
            >
